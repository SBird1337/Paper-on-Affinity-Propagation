...
%DIF DEL se-scientific-affinity-paper_v1.tex   Mon Jan 13 17:31:22 2020
%DIF ADD se-scientific-affinity-paper.tex      Mon Jan 13 17:31:24 2020
...
\begin{definition}\label{def:exemplar}
	An exemplar is a \DIFdelbegin \DIFdel{datapoint }\DIFdelend \DIFaddbegin \DIFadd{data point }\DIFaddend that is selected as the center of a cluster.
\end{definition}

\pagebreak
\section{Affinity Propagation}

Affinity Propagation works by passing messages between individual nodes in order to \DIFdelbegin \DIFdel{cummulate evidence of responsability }\DIFdelend \DIFaddbegin \DIFadd{cumulative evidence of responsibility }\DIFaddend and availability. The input parameter is the similarity of all nodes. This information is stored in matrices. (See Definition \ref{def:matrices}) Each entry of such a matrix encodes information about point $x_i$ and $x_k$ in the \DIFdelbegin \DIFdel{dataset}\DIFdelend \DIFaddbegin \DIFadd{data set}\DIFaddend . Affinity Propagation can be used to identify clusters, and their respective exemplars.
...
\[
	A(i,k) + R(i,k)
\]
\DIFdelbegin \DIFdel{eighter }\DIFdelend \DIFaddbegin \DIFadd{either }\DIFaddend identifies $x_i$ as an exemplar if $i=k$ or $x_k$ as an exemplar for $x_i$. \cite{frey2007clustering} For reasonable results the algorithm may be terminated after a fixed number of iterations, after changes in messages fall below a a \DIFdelbegin \DIFdel{treshold}\DIFdelend \DIFaddbegin \DIFadd{threshold}\DIFaddend , or after the local decisions stay constant for some number of iterations. \cite{frey2007clustering}
\DIFaddbegin 

\DIFadd{A visualized form of the message-passing procedure can be seen in Figure \ref{fig:iterations}, where you can see how each point gradually chooses its exemplar.
}\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{../figures/frey-dueck-cluster.jpg}
	\caption{\DIFaddFL{Visualized message-passing procedure \mbox{%DIFAUXCMD
\cite{frey2007clustering} }\hspace{0pt}%DIFAUXCMD
showing the likelihood of a point being an exemplar, and its exemplar-relation to other points.}}
	\label{fig:iterations}
\end{figure}
\pagebreak
\DIFaddend \subsection{Numerical Oscillations}
